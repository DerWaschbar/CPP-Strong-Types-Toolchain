...
std::string
HeaderOnlySerializer::serializeStrongType(const stt::StrongType& strongType,
...
HeaderOnlySerializer::serializeBinaryOperation(const stt::BinaryOperation& binaryOperation) {
    ...
    Template defTemplate = binaryOperation.isBinaryAssignment() ? Template::T_BinaryAssignmentOp
                                                                : Template::T_BinaryOp;
    Template headerTemplate = binaryOperation.isBinaryAssignment()
                                      ? Template::T_BinaryAssignmentOpHeader
                                      : Template::T_BinaryOpHeader;

    return {inja::render((&templateManager)->getTemplate(headerTemplate), opJson),
            inja::render((&templateManager)->getTemplate(defTemplate), opJson)};
}

std::pair<std::string, std::string>
HeaderOnlySerializer::serializeUnaryOperation(const stt::UnaryOperation& unaryOperation) {
    nlohmann::json opJson;
...
std::string
HeaderOnlySerializer::serializeIncludes(const std::vector<stt::StrongType>& includedTypes) {
    std::string includes;
